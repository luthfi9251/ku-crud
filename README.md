...
### Project layout
    ...
    main.go               production single binary: flags, go:embed of web/dist,
                          serves SPA + API (stays at root — go:embed cannot
                          reach parent directories)
    cmd/dev/              dev entry: API only, no embedded SPA (run frontend via vite)
...
Run each side on its own — no rebuild of the SPA needed while coding:
    ...
    # terminal 1 — API only (cmd/dev), on :8080
    make dev
    # or: go run ./cmd/dev -addr :8080 -data /tmp/dev.db

    # terminal 2 — frontend with hot reload, on :5173 (proxies /api → :8080)
    cd web && npm run dev

Open http://localhost:5173. API changes: restart `cmd/dev`. Frontend changes:
vite hot-reloads.
...
### Backend
    ...
    go run ./cmd/dev -addr :8080 -data /tmp/dev.db

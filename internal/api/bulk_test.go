...
func TestBulkDelete(t *testing.T) {
	...
	w := do(s, "POST", "/api/data/"+ordTok+"/rows/bulk-delete", `{"keys":["`+keys+`"]}`, c)
	...
	w = do(s, "POST", "/api/data/"+custTok+"/rows/bulk-delete", `{"keys":["`+keys+`"]}`, c)
	if w.Code != 200 {
...
func TestBulkDeleteConflictPartialSuccess(t *testing.T) {
	...
	w = do(s, "GET", "/api/data/"+custTok+"/rows", "", c)
...
func TestBulkDeleteGrantsAndCaps(t *testing.T) {
	...
	if w := do(s, "POST", "/api/data/"+tok+"/rows/bulk-delete", `{"keys":["`+engine.EncodeRowKey([]string{"1"})+`"]}`, reader); w.Code != 403 {
	...
	if w := do(s, "POST", "/api/data/"+tok+"/rows/bulk-delete", body, c); w.Code != 400 ||
		!strings.Contains(w.Body.String(), "BULK_TOO_LARGE") {
		t.Fatalf("cap = %d %s", w.Code, w.Body)
	}
	// empty keys → 400
	if w := do(s, "POST", "/api/data/"+tok+"/rows/bulk-delete", `{"keys":[]}`, c); w.Code != 400 {

...
func TestRowListRels(t *testing.T) {
	...
	w := do(s, "GET", "/api/data/"+defName(s, 2)+"/rows", "", c)
	if w.Code != 200 {
	...
	w = do(s, "GET", "/api/data/"+defName(s, 2)+"/rows/"+engine.EncodeRowKey([]string{"1"}), "", c)
	...
	w = do(s, "GET", "/api/data/"+defName(s, 2)+"/fkoptions/customer_id?search=jakarta", "", c)
	...
	w = do(s, "GET", "/api/data/"+defName(s, 2)+"/fkoptions/customer_id", "", c)
	...
	w = do(s, "GET", "/api/data/"+defName(s, 2)+"/rows?search=zzzznomatch", "", c)
	if w.Code != 200 || !strings.Contains(w.Body.String(), `"rows":[]`) {
		t.Fatalf("empty row list shape = %d %s", w.Code, w.Body)
	}
	w = do(s, "GET", "/api/data/"+defName(s, 2)+"/fkoptions/customer_id?search=zzzznomatch", "", c)
	if w.Code != 200 || !strings.Contains(w.Body.String(), `"rows":[]`) {
		t.Fatalf("empty fkoptions shape = %d %s", w.Code, w.Body)
	}
	// non-fk column → 404
	if w = do(s, "GET", "/api/data/"+defName(s, 2)+"/fkoptions/note", "", c); w.Code != 404 {
...
func TestRowListRelsRBAC(t *testing.T) {
	...
	w := do(s, "GET", "/api/data/"+defName(s, 2)+"/rows", "", reader)
	if w.Code != 200 {
	...
	if w = do(s, "GET", "/api/data/"+defName(s, 2)+"/fkoptions/customer_id", "", reader); w.Code != 403 {
	...
	w = do(s, "GET", "/api/data/"+defName(s, 2)+"/rows", "", rw)
	if !strings.Contains(w.Body.String(), `"rels":{"customer_id"`) {
		t.Fatalf("granted rels missing: %s", w.Body)
	}
	w = do(s, "GET", "/api/data/"+defName(s, 2)+"/fkoptions/customer_id", "", rw)
	if w.Code != 200 {
...
func TestFKWriteAndDeleteProtection(t *testing.T) {
	...
	w := do(s, "POST", "/api/data/"+defName(s, 2)+"/rows", body, c)
	...
	if w = do(s, "POST", "/api/data/"+defName(s, 2)+"/rows", `{"note":"o9","customer_id":3}`, c); w.Code != 200 {
		t.Fatalf("valid fk create = %d %s", w.Code, w.Body)
	}

	// update to dangling fk → 400
	w = do(s, "PUT", "/api/data/"+defName(s, 2)+"/rows/"+engine.EncodeRowKey([]string{"1"}),
		`{"customer_id":999}`, c)
	if w.Code != 400 || !strings.Contains(w.Body.String(), "referenced row not found") {
	...
	if w = do(s, "DELETE", "/api/data/"+defName(s, 2)+"/rows/"+engine.EncodeRowKey([]string{"4"}), "", c); w.Code != 200 {
		t.Fatalf("delete order o9 = %d %s", w.Code, w.Body)
	}
	if w = do(s, "DELETE", "/api/data/"+defName(s, 1)+"/rows/"+engine.EncodeRowKey([]string{"3"}), "", c); w.Code != 200 {
		t.Fatalf("delete unreferenced customer = %d %s", w.Code, w.Body)
	}

	// null fk on create is fine
	if w = do(s, "POST", "/api/data/"+defName(s, 2)+"/rows", `{"note":"o10"}`, c); w.Code != 200 {
...
func TestDeleteProtectionMultiRow(t *testing.T) {
	...
	w := do(s, "DELETE", "/api/data/"+defName(s, itemsID)+"/rows/"+engine.EncodeRowKey([]string{"g1"}), "", c)
	...
	if w = do(s, "DELETE", "/api/data/"+defName(s, refsID)+"/rows/"+engine.EncodeRowKey([]string{"1"}), "", c); w.Code != 200 {
	...
	w = do(s, "GET", "/api/data/"+defName(s, itemsID)+"/rows", "", c)
...
func TestFKOptionsNonKeyRefColumn(t *testing.T) {
	...
	w := do(s, "GET", "/api/data/"+defName(s, ordersID)+"/fkoptions/customer_name", "", c)

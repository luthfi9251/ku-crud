...
func TestImportPreviewRunsBeforeHooks(t *testing.T) {
	...
	w := doImport(t, s, c, "/api/tables/"+tdTok(s, 1)+"/import/preview", "name,status\nalice,sunny\nbob,sunny\n", nil)
	if w.Code != 200 {
...
func TestImportApplyHookRejectAndEnqueue(t *testing.T) {
	...
	w := doImport(t, s, c, "/api/tables/"+tdTok(s, 1)+"/import/apply",
		"name,status\nalice,sunny\nbob,sunny\n", map[string]string{
	...
	if w := do(s, "GET", "/api/tables/"+tdTok(s, 1)+"/rows?search=bob", "", c); strings.Contains(w.Body.String(), "bob") {
